...
import { Model, sql } from '../../../src/models/model.js';
...
sql`DROP TABLE IF EXISTS covid_data_state_abbreviation_original_source`,
sql`
...
sql`DROP TABLE IF EXISTS covid_data_state_governor_political_party_original_source`,
sql`
...
sql`DROP TABLE IF EXISTS covid_data_state_population_original_source`,
sql`
...
sql`DROP TABLE IF EXISTS covid_data_covid_tracking_us_original_source`,
sql`
    CREATE TABLE IF NOT EXISTS covid_data_covid_tracking_us_original_source
    (cdctuos_id SERIAL PRIMARY KEY
    ,UNIQUE (cdctuos_date) -- UNIQUE comes with an automatic unique index
    ,cdctuos_date INTEGER NOT NULL -- ex: 20200101
...
`,
sql`DROP TABLE IF EXISTS covid_data_covid_tracking_states_original_source`,
sql`
    CREATE TABLE IF NOT EXISTS covid_data_covid_tracking_states_original_source
    (cdctsos_id SERIAL PRIMARY KEY
    ,UNIQUE (cdctsos_date, cdctsos_state)
...
// ttd: exit(1) in this last catch block? really need the last catch block? I'm never going to throw there, just exi